...
def main():
    ...
    print("Agent started (streaming mode). Type 'exit' or 'quit' to stop.")
    ...
    while True:
        try:
            ...
            if pending_interrupt:
                # For interrupt resumes, stream the response
                print("Agent: ", end="", flush=True)
                for msg, metadata in graph.stream(
                    Command(resume=user_input), config, stream_mode="messages"
                ):
                    if isinstance(msg, AIMessageChunk) and msg.content and metadata["langgraph_node"] == "llm_call":
                ...
                for msg, metadata in graph.stream(
                    ...
                    # Only print content tokens from the LLM node (not tool calls/results)
                    if isinstance(msg, AIMessageChunk) and msg.content and metadata["langgraph_node"] == "llm_call":
                        print(msg.content, end="", flush=True)
                print()  # newline after streaming completes
            
            # Check if the agent paused for human-in-the-loop approval
